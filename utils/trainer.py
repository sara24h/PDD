...
class PDDTrainer:
    def __init__(self, student, teacher, train_loader, test_loader, device, args, rank, world_size, checkpoint=None):
        ...
        self.optimizer = torch.optim.SGD(
            [
                ...
                {'params': mask_params, 'lr': args.lr * 0.015, 'weight_decay': 0.0}
            ],
    ...
    def train(self, start_epoch=0):
        ...
        for epoch in range(start_epoch, self.args.epochs):
            ...
            # 🔥 اصلاح: استفاده از dtype=torch.long برای شمارش دقیق
            train_loss = torch.tensor(0.0, device=self.device)
            kd_loss_total = torch.tensor(0.0, device=self.device)
            ...
            for batch_idx, (inputs, targets) in enumerate(pbar):
                ...
                # 🔥 اصلاح: تبدیل به long برای شمارش صحیح
                correct += predicted.eq(targets).sum().long()
    ...
    def evaluate(self):
        self.student.eval()
        # 🔥 اصلاح: استفاده از long برای evaluate هم
        correct = torch.tensor(0, dtype=torch.long, device=self.device)
        total = torch.tensor(0, dtype=torch.long, device=self.device)
        with torch.no_grad():
    ...
    def _calculate_pruning_ratio(self):
        ...
        if total_channels == 0:
            return 0.0
        return 100.0 * pruned_channels / total_channels
